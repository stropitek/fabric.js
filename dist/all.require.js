...
(function() {
    ...
    function arcToSegments(x, y, rx, ry, large, sweep, rotateX, ox, oy) {
        ...
        var xc = .5 * (coords.x0 + coords.x1) - sfactor * (coords.y1 - coords.y0);
        var yc = .5 * (coords.y0 + coords.y1) + sfactor * (coords.x1 - coords.x0);
...
(function() {
    var utilMin = fabric.util.array.min, utilMax = fabric.util.array.max;
    fabric.ExtPencilBrush = fabric.util.createClass(fabric.BaseBrush, {
        initialize: function(canvas) {
            this.canvas = canvas;
            this._points = [];
        ...
        onMouseMove: function(pointer) {
            this._captureDrawingPath(pointer);
            this.canvas.clearContext(this.contextBrush);
            this._render();
        },
        onMouseUp: function() {
            this._finalizeAndAddPath();
        ...
        _prepareForDrawing: function(pointer) {
            ...
            this._reset();
            this._addPoint(p);
            this.contextBrush.moveTo(p.x, p.y);
        },
        _addPoint: function(point) {
            this._points.push(point);
        },
        ...
        convertPointsToSVGPath: function(points, minX, maxX, minY) {
            ...
            for (var i = 1, len = points.length; i < len; i++) {
                ...
                var midPoint = p1.midPointFrom(p2);
                if (!patherr) path.push("Q ", p1.x, " ", p1.y, " ", midPoint.x, " ", midPoint.y, " ");
                p1 = new fabric.Point(points[i].x - minX, points[i].y - minY);
                if (i + 1 < points.length) {
                    p2 = new fabric.Point(points[i + 1].x - minX, points[i + 1].y - minY);
                }
        ...
        _finalizeAndAddPath: function() {
            ...
            var originLeft = this.box.minx + (this.box.maxx - this.box.minx) / 2;
            var originTop = this.box.miny + (this.box.maxy - this.box.miny) / 2;
            this.contextBrush.arc(originLeft, originTop, 3, 0, Math.PI * 2, false);
            var path = this.createPath(pathData);
            path.set({
                left: originLeft,
                top: originTop
...
(function() {
    ...
    fabric.Canvas = fabric.util.createClass(fabric.StaticCanvas, {
        ...
        _setupCurrentTransform: function(e, target) {
            if (!target) return;
            var corner = target._findTargetCorner(e, this._offset), pointer = getPointer(e, target.canvas.upperCanvasEl), action = this._getActionFromCorner(target, corner), origin = this._getOriginFromCorner(target, corner);
        ...
        _scaleObjectEqually: function(localMouse, target, transform) {
            ...
            var lastDist = (target.height + target.strokeWidth) * transform.original.scaleY + (target.width + target.strokeWidth) * transform.original.scaleX;
...
(function() {
    ...
    fabric.util.object.extend(fabric.Canvas.prototype, {
        ...
        _onMouseDownInDrawingMode: function(e) {
            ...
            if (!this.isAltFreehandMode) this.freeDrawingBrush.onMouseDown(this.getPointer(e));
        ...
        _onMouseMoveInDrawingMode: function(e) {
            if (this._isCurrentlyDrawing) {
                ...
                if (!this.isAltFreehandMode) this.freeDrawingBrush.onMouseMove(pointer);
            }
        ...
        _onMouseUpInDrawingMode: function(e) {
            ...
            if (!this.isAltFreehandMode) this.freeDrawingBrush.onMouseUp();
            this.fire("mouse:up", {
                e: e
            });
        },
...
(function() {
    var getPointer = fabric.util.getPointer, degreesToRadians = fabric.util.degreesToRadians, isVML = typeof G_vmlCanvasManager !== "undefined";
    fabric.util.object.extend(fabric.Object.prototype, {
...
(function(global) {
    ...
    fabric.Rect = fabric.util.createClass(fabric.Object, {
        ...
        _render: function(ctx) {
            var rx = this.rx || 0, ry = this.ry || 0, x = -this.width / 2, y = -this.height / 2, w = this.width, h = this.height, isInPathGroup = this.group && this.group.type === "path-group";
            ctx.beginPath();
...
(function(global) {
    ...
    fabric.Group = fabric.util.createClass(fabric.Object, fabric.Collection, {
        ...
        _getBounds: function(aX, aY) {
            var minX = min(aX), maxX = max(aX), minY = min(aY), maxY = max(aY), width = maxX - minX || 0, height = maxY - minY || 0;
            return {
                width: width,
                height: height,
                left: minX + width / 2 || 0,
...
(function(global) {
    ...
    fabric.Image.filters.Convolute = fabric.util.createClass(fabric.Image.filters.BaseFilter, {
        ...
        applyTo: function(canvasEl) {
            var weights = this.matrix, context = canvasEl.getContext("2d"), pixels = context.getImageData(0, 0, canvasEl.width, canvasEl.height), side = Math.round(Math.sqrt(weights.length)), halfSide = Math.floor(side / 2), src = pixels.data, sw = pixels.width, sh = pixels.height;
            var w = sw;
            var h = sh;
...
(function() {
    ...
    fabric.IText = fabric.util.createClass(fabric.Text, fabric.Observable, {
        ...
        renderCursor: function(boundaries) {
            var ctx = this.ctx;
            ctx.save();
            var cursorLocation = this.get2DCursorLocation(), lineIndex = cursorLocation.lineIndex, charIndex = cursorLocation.charIndex, charHeight = this.getCurrentCharFontSize(lineIndex, charIndex);
            ctx.fillStyle = this.getCurrentCharColor(lineIndex, charIndex);
            ctx.globalAlpha = this._currentCursorOpacity;
            ctx.fillRect(boundaries.left + boundaries.leftOffset, boundaries.top + boundaries.topOffset, this.cursorWidth / this.scaleX, charHeight);
            ctx.restore();
        },
        renderSelection: function(chars, boundaries) {
            ...
            ctx.save();
            ctx.fillStyle = this.selectionColor;
            var cursorLocation = this.get2DCursorLocation(), lineIndex = cursorLocation.lineIndex, charIndex = cursorLocation.charIndex, textLines = this.text.split(this._reNewline), origLineIndex = lineIndex;
            for (var i = this.selectionStart; i < this.selectionEnd; i++) {
                if (chars[i] === "\n") {
                    ...
                    var charWidth = this._getWidthOfChar(ctx, chars[i], lineIndex, charIndex), lineOffset = this._getLineLeftOffset(this._getWidthOfLine(ctx, lineIndex, textLines)) || 0;
                    if (lineIndex === origLineIndex) {
                        lineOffset = 0;
                    }
                    ctx.fillRect(boundaries.left + boundaries.leftOffset + lineOffset, boundaries.top + boundaries.topOffset, charWidth, this._getHeightOfLine(ctx, lineIndex));
        ...
        _renderChars: function(method, ctx, line, left, top, lineIndex) {
            ...
            var textLines = this.text.split(this._reNewline), lineWidth = this._getWidthOfLine(ctx, lineIndex, textLines), lineHeight = this._getHeightOfLine(ctx, lineIndex, textLines), lineLeftOffset = this._getLineLeftOffset(lineWidth), chars = line.split("");
    ...
    fabric.util.object.extend(fabric.IText.prototype, {
        ...
        removeStyleObject: function(isBeginningOfLine, index) {
            ...
            if (isBeginningOfLine) {
                var textLines = this.text.split(this._reNewline), textOnPreviousLine = textLines[lineIndex - 1], newCharIndexOnPrevLine = textOnPreviousLine.length;
                if (!this.styles[lineIndex - 1]) {
                    this.styles[lineIndex - 1] = {};
                }
...
fabric.util.object.extend(fabric.IText.prototype, {
    ...
    isDoubleClick: function(newPointer) {
        return this.__newClickTime - this.__lastClickTime < 500 && this.__lastPointer.x === newPointer.x && this.__lastPointer.y === newPointer.y;
    },
